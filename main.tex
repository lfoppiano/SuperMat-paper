...
In this paper, we present SuperMat (Superconductor Materials), an annotated corpus of linked data from scientific publications on superconductors. The corpus is composed of 142 articles, 16052 entities, and 1398 links, annotated for six categories: the names, classes, and properties of materials, and the links to their respective superconducting critical temperature (\textit{T\textsubscript{c}}), and parametric conditions such as applied pressure or measurement methods.
...
The quality of the annotations guidelines was ensured by satisfactory Inter Annotator Agreement (IAA) between the annotators and domain experts. 
SuperMat includes the dataset, the annotation guidelines, and the annotation support tools that use automatic suggestions to help minimise human errors.
\end{abstract}
